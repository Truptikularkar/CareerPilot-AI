...
class InterviewPrepState(TypedDict, total=False):
    ...
    _gemini_answers: List[InterviewAnswer]
...
def generate_questions_node(state: InterviewPrepState) -> Dict[str, Any]:
    logger.info("LangGraph Interview Node: Generating core interview questions & coding challenges...")
    analysis = state["job_analysis"]
    resume = state["tailored_resume"]
    seed = state["readiness_seed"]
    quotas = state["quotas"]
    ...
    coding_challenges = pack.get("coding_challenges", [])
    gemini_answers = pack.get("answers", [])

    return {
        "questions": questions,
        "coding_challenges": coding_challenges,
        "_gemini_answers": gemini_answers,
    }
...
def generate_answers_node(state: InterviewPrepState) -> Dict[str, Any]:
    ...
    # Merge answers generated by Gemini
    gemini_answers = state.get("_gemini_answers", [])
    if gemini_answers:
        existing_q_ids = {a.question_id for a in answers}
        for ga in gemini_answers:
            if ga.question_id not in existing_q_ids:
                answers.append(ga)
...
def export_artifacts_node(state: InterviewPrepState) -> Dict[str, Any]:
    ...
    coding_challenges = state.get("coding_challenges", [])
